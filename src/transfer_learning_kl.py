'''
This code file uses the MNIST dataset as the incremental
dataset and trains the central model, which was initially
trained on the SVHN dataset, within a federated incremental
learning framework based on knowledge distillation.
'''
...
'''
Step1: Load Pretrained Global Model
'''
...
'''
def federated_update(model, client_datasets, num_rounds=6, epochs=1, lr=0.01, lambda_kd=0.5):
    """
    Perform federated training with knowledge distillation (no replay from SVHN).

    Args:
        model: Pre-trained global model (SVHN).
    ...
    svhn_test_loader = data.DataLoader(svhn_test, batch_size=32, shuffle=False)
    ...
    for round_idx in range(num_rounds):
        ...
        for dataset in client_datasets:
            ...
            for _ in range(epochs):
                for inputs, labels in dataloader:
                    ...
                    with torch.no_grad():
                        old_outputs = global_model(inputs)
        ...
        mnist_acc, mnist_loss = evaluate(global_model, mnist_test_loader)
        svhn_acc, svhn_loss = evaluate(global_model, svhn_test_loader)

        print(
            f"Round {round_idx + 1}/{num_rounds} - "
            f"MNIST Acc: {mnist_acc:.4f}, Loss: {mnist_loss:.4f} | "
            f"SVHN Acc: {svhn_acc:.4f}, Loss: {svhn_loss:.4f}"
        )
